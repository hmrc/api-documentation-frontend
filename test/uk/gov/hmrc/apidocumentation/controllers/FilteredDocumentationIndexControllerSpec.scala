  ...
  "V2DocumentationController" when {
    "routing to the V2 index page" should {
      ...
      "render the filtered API list when doc type filter is road map and service guides but no category filter" in new Setup {
        ...
        // There are currently 25 Service Guides and 5 roadmaps so should be 30 results
        verifyPageRendered(pageTitle("API Documentation"), sideNavLinkRendered = false, breadcrumbs = List(apiDocsBreadcrumb), bodyContains = Seq("30 results "))(result)
      }
