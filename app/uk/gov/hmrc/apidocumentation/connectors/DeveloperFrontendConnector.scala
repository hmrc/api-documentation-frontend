...
class DeveloperFrontendConnector @Inject()(http: HttpClient, appConfig: ApplicationConfig,  metrics: Metrics) {
  ...
  def fetchNavLinks()(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[Seq[NavLink]] = metrics.record(api) {
    http.GET[Seq[NavLink]](s"$serviceBaseUrl/developer/user-navlinks")
  }

  def fetchTermsOfUsePartial()(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[HtmlPartial] = metrics.record(api) {
